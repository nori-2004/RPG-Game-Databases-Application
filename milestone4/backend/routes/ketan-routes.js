...
// K3: SELECTION - Search Characters
router.get('/characters/search', async (req, res) => {
    const chrType = trimInput(req.query.chr_type);
    const locationName = trimInput(req.query.location_name);
    const characterKind = trimInput(req.query.character_kind) || 'all';
    ...
    try {
        ...
        if (characterKind === 'all' || characterKind === 'NPC') {
            ...
            if (chrType) {
                npcQuery += ' AND TRIM(chr_type) = :chr_type';
                npcBinds.chr_type = chrType;
            }
            if (locationName) {
                npcQuery += ' AND TRIM(location_name) = :location_name';
                npcBinds.location_name = locationName;
            }

            npcQuery += ' ORDER BY TRIM(chr_name)';
        ...
        if (characterKind === 'all' || characterKind === 'PC') {
            ...
            if (chrType) {
                pcQuery += ' AND TRIM(chr_type) = :chr_type';
                pcBinds.chr_type = chrType;
            }

            pcQuery += ' ORDER BY TRIM(chr_name)';
...
router.get('/users/report', async (req, res) => {
    ...
    if (!fieldsParam) {
        return res.status(400).json({
            success: false,
            error: 'fields parameter is required (comma-separated: username, first_name, created_on, email)'
        });
    }
