...
router.delete("/users/:username", async (req, res) => {
  ...
  try {
    ...
    const saveFilesCount = getCountFromResult(
      await db.query(
    ...
    const achievementsCount = getCountFromResult(
      await db.query(
    ...
    const completedObjectivesCount = getCountFromResult(
      await db.query(
...
// JOIN - Achievement details with objectives (shows achievements earned by a user)
router.get("/achievements/details", async (req, res) => {
  ...
  try {
    ...
    if (username) {
      result = await db.query(
        `SELECT TRIM(am.achievement_name) AS ACHIEVEMENT_NAME,
                CAST(am.description AS VARCHAR2(500)) AS ACHIEVEMENT_DESCRIPTION,
                TRIM(e.username) AS USERNAME,
                TRIM(u.first_name) AS FIRST_NAME,
                TRIM(am.obj_name) AS OBJ_NAME,
                CAST(o.description AS VARCHAR2(500)) AS OBJ_DESCRIPTION,
                TRIM(o.location_name) AS LOCATION_NAME,
                TRIM(o.item_name) AS ITEM_REWARD,
                TO_CHAR(e.date_earned, 'YYYY-MM-DD') AS DATE_EARNED
         FROM Earned e
         JOIN Achievements_monitors am ON TRIM(e.achievement_name) = TRIM(am.achievement_name)
         JOIN Users u ON TRIM(e.username) = TRIM(u.username)
         JOIN Objectives o ON TRIM(am.obj_name) = TRIM(o.obj_name)
         WHERE TRIM(e.username) = :username
         ORDER BY TRIM(e.username), TRIM(am.achievement_name)`,
        { username }
      );
    } else {
      result = await db.query(
    ...
    const data = (result.rows || []).map((row) => {
      // Convert all values to plain strings/nulls to avoid circular refs
      const safeValue = (val) =>
        val === null || val === undefined ? null : String(val);
      return {
        achievement_name: safeValue(row.ACHIEVEMENT_NAME),
        achievement_description: safeValue(row.ACHIEVEMENT_DESCRIPTION),
        username: safeValue(row.USERNAME),
        first_name: safeValue(row.FIRST_NAME),
        obj_name: safeValue(row.OBJ_NAME),
        obj_description: safeValue(row.OBJ_DESCRIPTION),
        location_name: safeValue(row.LOCATION_NAME),
        item_reward: safeValue(row.ITEM_REWARD),
        date_earned: safeValue(row.DATE_EARNED),
      };
    });
    ...
    const errorMsg =
      err && err.message ? err.message : "Unknown database error";
    return res.status(500).json({
      success: false,
...
router.get("/users/active", async (req, res) => {
  ...
  if (Number.isNaN(threshold)) {
    return res.status(400).json({
      success: false,
      error: "min_savefiles is required and must be a number",
    });
  }
...
router.get("/users/completionists", async (req, res) => {
  ...
  if (!locationName) {
    return res
      .status(400)
      .json({ success: false, error: "location_name is required" });
  }
  ...
  try {
    ...
    const objectivesAtLocation = (objectivesResult.rows || []).map(
      (row) => row.OBJ_NAME
    );
    ...
    if (data.length === 0) {
      responseBody.message =
        "No users have completed all objectives at this location";
    }
...
// Debug - Check Earned table contents
router.get("/debug/earned", async (_req, res) => {
  try {
    const result = await db.query(
      `SELECT TRIM(achievement_name) AS ACHIEVEMENT_NAME,
              TRIM(username) AS USERNAME,
              TO_CHAR(date_earned, 'YYYY-MM-DD') AS DATE_EARNED
       FROM Earned
       ORDER BY username`
    );
    return res.json({
    ...
    return handleServerError(res, err, "Failed to fetch earned data");
  }
});

// Debug - Check Achievements_monitors table contents
router.get("/debug/achievements", async (_req, res) => {
  try {
    const result = await db.query(
      `SELECT TRIM(achievement_name) AS ACHIEVEMENT_NAME,
              TRIM(username) AS USERNAME,
              TRIM(obj_name) AS OBJ_NAME
       FROM Achievements_monitors
    ...
    return handleServerError(res, err, "Failed to fetch achievements data");
  }
});
